...
static void print_packet(int fd, void *pack, unsigned len, char *dir)
{
# ifdef DEBUG
...
int decaps_gre (int fd, int (*cb)(int cl, void *pack, unsigned len), int cl) {
  ...
  if ( (status = recv(fd, buffer, sizeof(buffer), 0)) < 0)
...
print_packet(2, buffer, status, "(inet)");
...
int encaps_gre (int fd, void *pack, unsigned len) {
  ...
  if (pack==NULL)
  {
    if (ack_sent != seq_recv) {
      ...
      return sendto(fd, &u.header, sizeof(u.header) - sizeof(u.header.seq), 0,
  ...
  print_packet(2, u.buffer, header_len+len, "(unix)" );
  // return write(fd, u.buffer, header_len+len);
  return sendto(fd, u.buffer, header_len+len, 0,
      (struct sockaddr *) &src_addr, sizeof(src_addr));
