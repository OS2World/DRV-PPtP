 ...
 *
 * $Id: pptp_ctrl.c,v 1.7 1997/12/16 15:20:06 cananian Exp $
 */
...
/* Local prototypes */
static void print_packet(int fd, void *pack, unsigned len);
...
void pptp_dispatch_ctrl_packet(PPTP_CONN * conn, void * buffer, size_t size)
{
  ...
  case PPTP_START_CTRL_CONN_RPLY:
	...
	if (ntoh16(packet->version) != PPTP_VERSION) {
	  log("Wrong PPTP-Version %d, expected %d.", ntoh16(packet->version), PPTP_VERSION);
	...
	if (ntoh8(packet->result_code) > 1) { /* some problem with start */
	  log("Result code was %d.", ntoh8(packet->result_code));
