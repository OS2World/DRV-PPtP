 ...
 *
 * $Id: pptp_callmgr.c,v 1.8 1997/12/15 12:25:00 cananian Exp $
 */
#include <signal.h>
#include <sys/time.h>
#include <sys/types.h>
...
# ifdef __EMX__
  inetaddr.s_addr = inet_addr(argv[1]);
  if (inetaddr.s_addr == -1)
    fatal("Invalid IP address: %s", argv[1]);
  printf("callmgr: %s --> 0x%x\n", argv[1], inetaddr.s_addr);
# else
...
# endif
  ...
  if ((unix_sock = open_unixsock(inetaddr)) < 0)
    fatal("Could not open local socket for %s", argv[1]);
