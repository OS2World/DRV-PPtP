...
int main(int argc, char *argv[])
{
	...
	if (argc < 3 || argc > 4) {
		fprintf(stderr,"Usage: %s [-T] logfile cmd\n",argv[0]);
		exit(1);
	}
	...
	/* if tracemode, set tracemode */
	if (tracemode) {
		int tracelvl = 1;
		if (ptioctl(master_fd,XTY_TRACE,(ULONG*)&tracelvl) == -1)
			error("your xf86sup.sys doesn't support TRACE mode");
	}
