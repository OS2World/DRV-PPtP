...
_qmutex	 dd	40 dup (0)	; mutex semaphores for tty driver and con driver
...
histgdthelper	dw	0	; GDT for MapMem
_histbufsz	dw	80	; history buffer
_kversion	dw	0	; kernel revision
_DATA		ends
...
_int3		proc	far
		...
		jz	intex
_realint3:	int	3			; perform an int3
intex:		ret
_int3		endp
...
; ULONG Vlock(DWORD [bp+6], DWORD [bp+10])
; Lock a memory segment and return its physical address
; arg1 = virtual address of memory segment
; arg2 = size of segment
...
_Vlock		proc	far
		...
		mov	eax, dword ptr [bp+6]	; load base address
                FlatToSel                       ; macro
                                                ; now ax has selector, bx off.
		mov	cx, ax 
                shl     ecx, 16
                mov     cx, bx                  ; ecx now has sel:off
		mov	Paddr, ecx              ; Save this for now
                xor     bl, bl                  ; wait flag
                mov     bh, 1                   ; lock type, long term
                mov     dl, DevHlp_Lock         ; perform it
                call    [_Device_Help]
                jc      err00                   ; carry set if error
 						; result is in ax:bx
		mov     dx, ax			; high part in dx
		mov     ax, bx

		jmp	short ok00		; exit ok
err00:		xor	ax,ax			; exit error, return 0L
		xor	dx,dx
ok00:		pop	ecx
		pop	ebx
		pop	esi
...
; int Vunlock(DWORD [bp+6])
; unlock a previously locked memory object
; arg1 is the lock handle
; return -1, if error
; return 0, if successful
;
_Vunlock	 	proc	far
		...
		mov	eax, dword ptr [bp+6]	; get lock handle
		mov	bx, ax
		shr	eax, 16
		mov	dl, DevHlp_Unlock	; free it
		call	[_Device_Help]
		jc	err11			; if carry ,error
		xor	ax,ax			; exit ok
		jmp	short ok11
err11:		or	ax,0ffffh		; exit error
ok11:		pop 	ebx
		pop	edx
		pop	bp
		ret
...
; ULONG VtoP (DWORD [bp+6])
; Returns the physical address of a virtual address (normally locked memory)
; arg1 = virtual address of memory segment
; returns the physical address of the locked memory segment
; or 0L, if error
;
_VtoP		proc	far
		assume cs:_TEXT, ds:DGROUP
		...
		mov	Paddr, ecx              ; Save this for now
                lds     si, Paddr
                mov     dl, DevHlp_VirtToPhys   ; get phys addr. of mem seg.
                                                ; This is where things go wrong!!!!
                call    [_Device_Help]         
                jc      err000                   ; carry set if error

						; result is in ax:bx
		mov     dx, ax			; high part in dx
		mov     ax, bx

		jmp	short ok000		; exit ok
err000:		xor	ax,ax			; exit error, return 0L
		xor	dx,dx
ok000:		pop	ds
		pop	ecx
		pop	ebx
...
; es:edi* GetGdt(void)
; return a 16:32 flat pointer to the GDT into es:edi
; this uses the undocumented public kernel variable Dos32FlatDS
; which provides the flat R0 data segment selector of the kernel
		extrn	Dos32FlatDS:abs
GetGDT		proc	far
		sgdt	fword ptr [gdtsave]
		mov	di, Dos32FlatDS
		mov	es, di
		mov	edi, dword ptr [gdtsave+2]
		ret
GetGDT		endp
...
_AllocBuf	proc	far
		...
		; Incompatibility: while with Warp4 kernel, the function
		; returns a DPL=0 selector, with Aurora, a DPL=0 selector
		; is produced. So patch the GDT selector to make it DPL=0
		; again
		;AURORA AURORA AURORA AURORA
		...
		call	[_Device_Help]
		jc	err13			; error if carry set

		mov	ax,cx			; move ECX -> dx:ax
		shr	ecx,16
		and	cx,0fff8h		; ensure DPL0 GDT selector
		mov	dx,cx
		; AURORA: patch the GDT to be really sure
		push	es
		call	GetGDT
		add	edi, ecx
		and	byte ptr es:[edi+5], 09fh	; DPL=0
		pop	es
		; AURORA
...
; returns a linear address of a history buffer
; or 0 if error
_AllocHist	proc	far
		push	bp
		mov	bp,sp
		...
		push	edi
		xor	ecx,ecx
		mov	cx, word ptr [_histbufsz]
		or	cx,cx
		jz	err21
		mov	eax, 00000011b	; <16MB,FIXED
		mov	edx, 0ffffffffh	; not used
...
; char far* MapHist(ULONG linaddr)
; return 0 if error
; remains valid until next context check
_MapHist	proc	far
		push	bp
		mov	bp,sp
		push	es
		...
		mov	ax, word ptr [histgdthelper]
		or	ax,ax		; do we have a helper already?
		jnz	map2		; yes
		push	ds
		pop	es				; ES:DI = addr of
		mov	di, offset histgdthelper	; histgdthelper
		mov	cx, 1				; one selector
		mov	dl, DevHlp_AllocGDTSelector	; get GDT selector
		call	[_Device_Help]
		jc	err19

map2:		mov	ax, histgdthelper		; selector to map
		mov	ebx, dword ptr [bp+6]		; linear address
		xor	ecx,ecx
		mov	cx, word ptr [_histbufsz]	; size
		mov	dl, DevHlp_LinToGDTSelector
		call	[_Device_Help]
		jc	err19
		xor	ax,ax
		mov	dx, histgdthelper		; return GDT:0
		jmp	short ok19

err19:		xor	dx,dx		; error, return NULL
		xor	ax,ax
...
; routine. 
; What we do here is pretty much foul: We circumvent this mechanism and
; create an own selector, calculate the offset of such a selector in the GDT, 
; and change it to point to the driver routine directly. Therefore this routine 
; must be run in ring 0 mode.
;
...
_acquire_gdt	proc	far
		...
		push	es
		push	edi
		call	GetGDT				; es:edi = ptr to GDT
		movzx	eax, word ptr [_io_gdt32]	; build offset into table
		and	eax, 0fffffff8h			; mask away DPL
		add	edi, eax			; build address

		mov	word ptr es:[edi], offset _io_call ; fix address off
		mov	word ptr es:[edi+2], cs		; fix address sel
		mov	word ptr es:[edi+4], 0ec00h	; a r0 386 call gate
		mov	word ptr es:[edi+6], 0000h	; high offset
		pop	edi
		pop	es
aexit:
