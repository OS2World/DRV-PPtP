...
extern ULONG APIENTRY DosFlatToSel(PVOID);
 ...
 * find_mapphys:
 *   search for the entry point in the mapping table that belongs to the
 *   same file id and has the same 'physaddr', return index or -1
 */
static int find_mapphys(USHORT sf, ULONG physaddr, ULONG vmaddr)
{
	int i;
	int idx = -1;

	for (i=0; i<NMAP; i++) {
		if (maps[i].sfnum == sf && maps[i].phys == physaddr)
			if (vmaddr == 0xffffffffl || maps[i].vmaddr == vmaddr)
				return i;
	}
	return -1;
}
